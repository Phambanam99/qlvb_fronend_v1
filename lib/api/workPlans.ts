...
export const workPlansAPI = {
   ...
   */
  createWorkPlan: async (workPlanData: Partial<WorkPlanDTO>): Promise<WorkPlanDTO> => {
   ...
   */
  updateWorkPlan: async (id: string | number, workPlanData: Partial<WorkPlanDTO>): Promise<WorkPlanDTO> => {
  ...
  approveWorkPlan: async (id: number | string, data: { comments?: string }): Promise<WorkPlanDTO> => {
    const response = await api.post(`/work-plans/${id}/approve`, data);
    return response.data;
  },

  rejectWorkPlan: async (id: number | string, data: { comments?: string }): Promise<WorkPlanDTO> => {
