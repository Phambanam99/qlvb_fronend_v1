...
import { workflowAPI, documentResponsesAPI } from "@/lib/api";
...
export default function DepartmentHeadReview({
  documentId,
  responseId,
  departmentId,
}: DepartmentHeadReviewProps) {
  ...
  const handleReject = async () => {
    ...
    try {
      await workflowAPI.rejectDocumentResponse(responseId, comments, null);
            ...
            {response?.attachments && response.attachments.length > 0 && (
              <div className="mt-4">
                ...
                <div className="space-y-1">
                  {response.attachments.map((file: any, index: number) => (
